...
async function embedDashboard(_ref) {
  ...
  async function mountIframe() {
    return new Promise(resolve => {
      ...
      const filterConfigUrlParams = filterConfigKeys.length > 0 ? "&" + filterConfigKeys.map(key => _const.DASHBOARD_UI_FILTER_CONFIG_URL_PARAM_KEY[key] + "=" + filterConfig[key]).join("&") : ""; // set up style values
