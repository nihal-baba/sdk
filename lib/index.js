...
async function embedDashboard(_ref) {
  ...
  function log() {
    if (debug) {
      for (
        var _len = arguments.length, info = new Array(_len), _key = 0;
        _key < _len;
        _key++
      ) {
        ...
        filterConfigKeys.length > 0
          ? "&" +
            filterConfigKeys
              .map(
                (key) =>
                  _const.DASHBOARD_UI_FILTER_CONFIG_URL_PARAM_KEY[key] +
                  "=" +
                  filterConfig[key]
              )
              .join("&")
  ...
  async function refreshGuestToken() {
    ...
    setTimeout(
      refreshGuestToken,
      (0, _guestTokenRefresh.getGuestTokenRefreshTiming)(newGuestToken)
    );
  }

  setTimeout(
    refreshGuestToken,
    (0, _guestTokenRefresh.getGuestTokenRefreshTiming)(guestToken)
  );
  ...
  const getDashboardPermalink = (anchor) =>
    ourPort.get("getDashboardPermalink", {
      anchor,
    });
